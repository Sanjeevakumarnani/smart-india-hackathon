...
export const AdminPanelPage: React.FC<AdminPanelPageProps> = ({
  ...
  const fetchHealth = async () => {
    ...
    try {
      const res = await apiFetch('/api/admin/system-health', { headers: authHeaders() });
      if (res.ok) {
        const data = await res.json();
        setHealthData(data);
      }
  ...
  const handleToggleComplaint = async (id: string, currentActive: boolean) => {
    try {
      await apiFetch(`/api/chief-complaints/${id}`, {
        method: 'PUT',
  ...
  const handleToggleLanguage = async (code: string, currentActive: boolean) => {
    try {
      await apiFetch(`/api/languages/${code}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
        body: JSON.stringify({ is_active: !currentActive }),
      });
  ...
  const handleDeactivateUser = async (id: string) => {
    ...
    try {
      await apiFetch(`/api/admin/users/${id}`, { method: 'DELETE', headers: authHeaders() });
      await fetchUsers();
    } catch {
      // Fallback
    }

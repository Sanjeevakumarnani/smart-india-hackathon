...
export const DoctorConsolePage: React.FC<DoctorConsolePageProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      sse = new EventSource(apiUrl('/api/sse/queue-updates'));
      sse.onmessage = (event) => {
        try {
  ...
  const handleConfirmReprioritize = async () => {
    ...
    try {
      const res = await apiFetch(`/api/queue/${targetToken.tokenId || (targetToken as any).id}/reprioritize`, {
        method: 'PATCH',
  ...
  const handleCallPatient = async (token: QueueToken) => {
    ...
    try {
      await apiFetch(`/api/queue/${id}/call`, { method: 'PATCH' });
  ...
  const handleOpenPatientConsole = async (token: QueueToken) => {
    ...
    try {
      const res = await apiFetch(`/api/encounters/by-token/${encodeURIComponent(lookupId)}`);
      if (res.ok) {
        const data = await res.json();
